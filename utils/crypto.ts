// 암호화/복호화 유틸리티 (커스텀 XOR + Base64)
// 백엔드 src/common/utils/crypto.util.ts 와 동일 로직 — 같은 키로 프론트/백 호환
...
export function encrypt(data: string, key: string): string {
  try {
    ...
    // 바이트 단위로 XOR 암호화 (인덱스는 length로 경계가 보장됨)
...
export function decrypt(encryptedData: string, key: string): string {
  try {
    ...
    // 바이트 단위로 XOR 복호화 (인덱스는 length로 경계가 보장됨)
...
/**
 * 키 → 32자리 해시 문자열 (백엔드 crypto.util.ts 와 동일 로직)
 */
