...
onMounted(async () => {
  ...
  if (!isEditMode.value) {
    await navigateTo('/bookings/performance-management/create')
    return
  }
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isEditMode.value && performanceId.value) {
      ...
      setTimeout(() => {
        navigateTo(`/bookings/performance-management?id=${performanceId.value}`)
      }, 1500)
    }
  } catch (error: any) {
      ...
      <form @submit.prevent="handleSubmit" class="form">
        ...
        <div class="form-actions">
          <button
            ...
            @click="() => navigateTo('/bookings/performance-management')"
          >
            취소
          </button>
